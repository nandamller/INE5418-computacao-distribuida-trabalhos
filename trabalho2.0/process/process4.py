...
PORT = int(os.getenv("PORT", "6064"))
PRIMARY_ID = int(os.getenv("PRIMARY_ID", "6061"))

DEFAULT_TOPOLOGY = '{"6061":["process1",7061],"6062":["process2",7062],"6063":["process3",7063],"6064":["process4",7064],"6065":["process5",7065]}'
CLUSTER_TOPOLOGY = os.getenv("CLUSTER_TOPOLOGY", DEFAULT_TOPOLOGY)


class Process4(BaseProcess, VRNode):
    """Réplica VR. Ver Process1 para a explicação da herança múltipla."""

    def __init__(self, host: str, port: int, topology: dict, primary_id: int):
...
if __name__ == '__main__':
    ...
    p4 = Process4(host=HOST, port=PORT, topology=topology, primary_id=PRIMARY_ID)
    p4.start()